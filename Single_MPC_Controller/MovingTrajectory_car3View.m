function MovingTrajectory_car3View()
...
%% Plot the predicted trajectory of ego vehicle 3
EV_predicted_car3=predicted_states_EV{3}
TV_assumed_car1=assumeded_states_TV{1}
TV_assumed_car1=assumeded_states_TV{2}

% predicted_states of ego vehicle 3
position_x_EV=predicted_states_EV{3}(1,1);
position_y_EV=predicted_states_EV{3}(1,2);
predicted_positions_x_EV=predicted_states_EV{3}(:,1);
predicted_positions_y_EV=predicted_states_EV{3}(:,2);

% plot trajectory of ego vehicle 3
plot(position_x_EV', position_y_EV','o','linewidth',3,'color',colors_vehicles(3,:));
hold on
EV3=plot(predicted_positions_x_EV', predicted_positions_y_EV','-.','linewidth',1,'color',colors_vehicles(3,:));
% hold on
...
%% figure setting
start_x=position_x_EV-50;
end_x=position_x_EV+150;
title(sprintf('Moving trajectories from the view of vehicle 3'),'interpreter','latex','FontSize',fontsize_labels)
...
hold on;
saveas(gcf,sprintf('Moving Trajectories from the view of vehicle 3 FIG%d.fig',iterations_step));
saveas(gcf,sprintf('MovingTrajectoryOfvehicle3FIG%d.png',iterations_step));
set(EV3,'Visible','off');
if isempty(assumeded_states_TV{1})==0
set(TV1,'Visible','off');
else
