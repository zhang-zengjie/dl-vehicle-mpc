function [f, A_d, B_d]=fAB(T,id_vehicle,params_vehicles,states_vehicles,control_vehicles)
...
%  - f, A, B according to:
states_vehicles_id_vehicle=states_vehicles{id_vehicle};
control_vehicles_id_vehicle=control_vehicles{id_vehicle};

X = states_vehicles_id_vehicle(end,1);
Y = states_vehicles_id_vehicle(end,2);
psi = states_vehicles_id_vehicle(end,3);
v = states_vehicles_id_vehicle(end,4);  
a = 0;  
df = 0;
...
a = control_vehicles_id_vehicle(end-1,1);  
df = control_vehicles_id_vehicle(end,1);
...
A_d=[1, 0, -T*v*sin(z_1), T*cos(z_1)-z_2*sin(z_1)/(2*z_4);
0, 1, T*v*cos(z_1), T*sin(z_1)+z_2*cos(z_1)/(2*z_4);    
0, 0, 1, T*tan(df)/z_4;
0, 0, 0, 1];


B_d=[T^2*cos(z_1)/2, -T^2*v*z_7*sin(z_1)/2-z_8*sin(z_1)/z_9;
...
% z_8=T*lr*z_5;
% 
% A_d=[1, 0, -T*v*sin(z_1), T*v*cos(z_1)-z_2*sin(z_1)/(2*z_4);
% 0, 1, T*v*cos(z_1), T*v*sin(z_1)+z_2*cos(z_1)/(2*z_4);    
% 0, 0, 1, T*tan(df)/z_4;
% 0, 0, 0, 1];
% 
% 
% B_d=[T^2*cos(z_1)/2, -T^2*v*z_7*sin(z_1)/2-sin(z_1)/z_4;
